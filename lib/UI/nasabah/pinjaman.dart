...
class _PinjamanScreenState extends State<PinjamanScreen> {
  ...
  void initState() {
    ...
    Future.microtask(() {
      if (mounted) {
        Provider.of<PinjamanProvider>(context, listen: false).fetchPinjaman();
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<PinjamanProvider>(
      ...
      final punyaPinjamanAktif = pinjamanProvider.pinjaman.any((pinjaman) {
        return pinjaman['status'] == 'active';
      });
      double? ratioPinjaman;
      int persentasePinjaman = 0;

      if (punyaPinjamanAktif) {
        [pinjaman] = pinjamanProvider.pinjaman;
        ratioPinjaman = pinjaman['besar-pinjaman'] / 10000000;
        persentasePinjaman = (ratioPinjaman! * 100).toInt();
      }
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: CircularProgressIndicator(
                            ...
                            value: punyaPinjamanAktif ? ratioPinjaman : 0,
                            color: Colors.white,
                            backgroundColor:
